...
//
//    JdbcDataWriter("jdbc:postgresql://fijimf.com:5432/postgres","data_test", properties).writeDataFrame(frame)
//  }
//}
...
object LocalS3CsvRunner extends DataLoader {
  ...
  override def readDataFrame(session: SparkSession): DataFrame =
    session
      .read
      .option("header", "true")
      .csv("s3://fijimf-loans/sample.csv")
  ...
  override def writeDataFrame(frame: DataFrame): Unit = {
    ...
    properties.setProperty("password","password")
    properties.setProperty("driver","org.postgresql.Driver")
    JdbcDataWriter("jdbc:postgresql://fijimf.com:5432/peeriq_test","data_test", properties).writeDataFrame(frame)
  }
}
